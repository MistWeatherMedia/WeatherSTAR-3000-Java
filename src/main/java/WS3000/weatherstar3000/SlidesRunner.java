...
public class SlidesRunner extends Scene {
	...
	final String prefix = now.format(DateTimeFormatter.ofPattern("E MMM"));
	...
	ArrayList<String> tiMainLines = new ArrayList<>();
	private GameObject[] tiPool = new GameObject[288];
	...
	int slideIdx = 0;
	ArrayList<String> slideLineup;
	ArrayList<Float> slideDurations;
	boolean loop;
	...
	public void init() {
		...
		try {
			slideLineup = utl.loadSegments(Main.selectedFlavor);
		} catch (IOException e) {
			System.err.println("Exception loading flavor segments");
			System.exit(100);
			e.printStackTrace();
		}
		
		try {
			slideDurations = utl.loadDurations(Main.selectedFlavor);
		} catch (IOException e) {
			System.err.println("Exception loading flavor durations");
			System.exit(101);
			e.printStackTrace();
		}
	...
	private void initPools() {
	    ...
	    for (int i = 0; i < tiPool.length; i++) {
	    	tiPool[i] = new GameObject("tiPool" + i, new Transform(new Vector2f()), 0);
	    	tiPool[i].addComponent(new SpriteRenderer(new Vector4f(1,1,1,1)));
	    }
	    ...
	    for (int i = 0; i < dtPool.length; i++) {
	    	dtPool[i] = new GameObject("dtPool" + i, new Transform(new Vector2f()), 2);
	    	dtPool[i].addComponent(new SpriteRenderer(new Vector4f(1,1,1,1)));
	    }
	    
	    for (int i = 0; i < ctPool.length; i++) {
	    	ctPool[i] = new GameObject("ctPool" + i, new Transform(new Vector2f()), 2);
	...
	public void update(float dt) {
		...
		slideLength = slideDurations.get(slideIdx);
		slideChooser();
		
		if (travelForecastActive) {
			if (Window.secFrameCount % 2 == 0) {
				travelOffset--;
				tinterval++;
				if (tinterval == 3) {
					travelOffset--;
	...
	public void dateTime() {
		now = ZonedDateTime.now(Main.timeZone);
	...
	public void ctData() {
		...
		if (ctData.condition.equals("NO CURRENT REPORT")) {
	...
	public void localForecast() {
        ...
        if (!slideInit) {
        	...
        	if (!lfData.bulletin.get(0).equals("NOBULLETIN")) {
            ...
            if (lfbMainLines.get(0).equals("NOTHING") && forecastOneLines.size() < 8) {
            	lfMainLines.add(utl.ljust("", 32, " "));
                linesPassed++;
            } else {
            	if (!lfbMainLines.get(0).equals("NOTHING")) {
            	...
            	if (forecastOneLines.size() < 7 - linesPassed) {
            		lfMainLines.add(utl.ljust("", 32, " "));
                    linesPassed++;
            	}
            }
            
            //then add first forecast
	...
	public void regionalForecast() {
		...
		if (!slideInit) {
			...
			} else if (rfData.first.equals("H")) {
				rfMainLines.add("CITY           WEATHER    HI LOW");
				for (int i = 0; i < rfData.locName.size(); i++) {
					rfMainLines.add(utl.ljust(rfData.locName.get(i), 14, " ") + " " + utl.ljust(rfData.condition.get(i), 10, " ") + rfData.high.get(i) + " " + rfData.low.get(i));
				}
			}
	...
	public void outlook() {
		...
		if (!slideInit) {
			...
			olMainLines.add(utl.ljust(" ", 32, " "));
			olMainLines.add("        30 DAY OUTLOOK          ");
			olMainLines.add(oData.month);
			olMainLines.add(utl.ljust(" ", 32, " "));
			olMainLines.add("TEMPERATURES:  " + oData.temperature);
			olMainLines.add(utl.ljust(" ", 32, " "));
			olMainLines.add("PRECIPITATION: " + oData.precipitation);
	...
	public void tides() {
		...
		if (!slideInit) {
			Tides tiData = Main.tides;
			
			tiMainLines.clear();
			
			tiMainLines.add("     LOCAL TIDE INFORMATION     ");
			tiMainLines.add(utl.ljust(" ", 32, " "));//half space
			tiMainLines.add(tiData.locName.get(0));
			tiMainLines.add("HIGHS: " + tiData.highTide.get(0));
			tiMainLines.add("LOWS:  " + tiData.lowTide.get(0));
			tiMainLines.add(tiData.locName.get(1));
			tiMainLines.add("HIGHS: " + tiData.highTide.get(1));
			tiMainLines.add("LOWS:  " + tiData.lowTide.get(1));
			tiMainLines.add("   SUNRISE" + tiData.sunrise + "  SET" + tiData.sunset + "   ");
        ...
        for (int row = 0; row < tiMainLines.size(); row++) {
        	int lineHeight = row == 1 ? 22 : 40;
        	String line = tiMainLines.get(row);
        	for (int i = 0; i < line.length(); i ++) {
        		if (poolIdx >= tiPool.length) break;
        		
        		char c = line.charAt(i);
        		
        		GameObject go = tiPool[poolIdx++];
        		go.transform.position.set(textX, textY);
        		go.transform.scale.set(18, 32);
	...
	public void travelForecast() {
	    ...
	    if (!travelDataInit) {
	        ...
	        } else if ((tfData.first.equals("H"))) {
	        	travelMainLines.add("CITY           WEATHER    HI LOW");
	        	for (int i = 0; i < tfData.locName.size(); i++) {
	        		travelMainLines.add(utl.ljust(tfData.locName.get(i), 15, " ") + 
	                               utl.ljust(tfData.condition.get(i), 10, " ") + 
	                               utl.rjust(tfData.high.get(i), 3, " ") + 
	                               utl.rjust(tfData.low.get(i), 4, " "));
	        	}
	        }
	...
	public void slideChooser() {
		//draw slide
		String slide = slideLineup.get(slideIdx);
