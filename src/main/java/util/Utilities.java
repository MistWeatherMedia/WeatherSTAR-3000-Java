...
public class Utilities {
    ...
    public ArrayList<String> loadSegments(String flavor) throws IOException {
    	String json = readJsonFromResource("/resources/json/SlidesManager.json");
    	JSONObject jsonObject = new JSONObject(json);
    	
    	JSONArray fl = jsonObject.getJSONArray(flavor);
    	
    	ArrayList<String> segments = new ArrayList<>();
    	
    	for (int i = 0; i < fl.length(); i++) {
    		segments.add(fl.getJSONObject(i).getString("segment"));
    	}
    	
    	return segments;
    }
    
    public ArrayList<Float> loadDurations(String flavor) throws IOException {
    	String json = readJsonFromResource("/resources/json/SlidesManager.json");
    	JSONObject jsonObject = new JSONObject(json);
    	...
    	for (int i = 0; i < fl.length(); i++) {
    		segments.add(fl.getJSONObject(i).getFloat("duration"));
    	}
